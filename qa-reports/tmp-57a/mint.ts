/**
 * SEC-01 live verify (57-a) — mint نشست برای ۳ نقش: SUPER_ADMIN / SUPPORT_AGENT / STORE_MANAGER
 * هش رمز از ادمین موجود کپی می‌شود (بدون دانستن/تغییر رمز ادمین واقعی).
 * اجرا: cd /home/z/my-project/towel-parima && DATABASE_URL="$(bash scripts/pg.sh url)" bun qa-reports/tmp-57a/mint.ts mint|cleanup
 */
...
const emailOf = (r: string) => `sec01-57a+${r.toLowerCase()}@prima.test`;
...
async function mint() {
  ...
  for (const roleName of ROLES) {
    ...
    const user = await db.user.upsert({
      ...
      create: {
        email,
        phone: `0913570${Math.floor(Math.random() * 9000 + 1000)}`,
        name: `تست 57a ${roleName}`,
        roleId: role.id,
        isActive: true,
        passwordHash: admin.passwordHash, // کپی هش ادمین موجود — نه رمز جدید
      },
    });
    const token = `sec57a_${roleName.toLowerCase()}_${"b".repeat(38)}`;
...
async function cleanup() {
  ...
  console.log(JSON.stringify({ mintedSessionsDeleted: users.length, mintedUsersDeleted: res.count }));
}
