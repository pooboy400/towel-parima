...
-- CreateIndex
CREATE INDEX "JournalPost_status_publishedAt_idx" ON "JournalPost"("status", "publishedAt");
...
-- CreateIndex
CREATE UNIQUE INDEX "MediaObject_sha256_key" ON "MediaObject"("sha256");
...
-- AddForeignKey
ALTER TABLE "MediaObject" ADD CONSTRAINT "MediaObject_uploadedBy_fkey" FOREIGN KEY ("uploadedBy") REFERENCES "User"("id") ON DELETE SET NULL ON UPDATE CASCADE;
...
-- ------------------------------------------------------------------
-- بخش ۴.۲/۴.۶ سند معماری — قیدها و ایندکس‌های خارج از قابلیت Prisma
-- ------------------------------------------------------------------

-- CHECK واریانت: موجودی هرگز منفی نمی‌شود (بخش ۴.۲)
ALTER TABLE "Variant"
  ADD CONSTRAINT "variant_stock_nonnegative" CHECK ("stock" >= 0),
  ADD CONSTRAINT "variant_reserved_nonnegative" CHECK ("reserved" >= 0),
  ADD CONSTRAINT "variant_available_nonnegative" CHECK ("stock" - "reserved" >= 0);

-- CHECK مقدار عددی پول و تخفیف واریانت
ALTER TABLE "Variant"
  ADD CONSTRAINT "variant_price_nonnegative" CHECK ("price" >= 0),
  ADD CONSTRAINT "variant_compare_at_price" CHECK ("compareAtPrice" IS NULL OR "compareAtPrice" > "price");

-- CHECK رتبه نظرات (1..5) و قیمت سفارش
ALTER TABLE "Review" ADD CONSTRAINT "review_rating_range" CHECK ("rating" BETWEEN 1 AND 5);
ALTER TABLE "Order" ADD CONSTRAINT "order_totals_nonnegative" CHECK ("grandTotal" >= 0 AND "subtotal" >= 0);

-- FTS فارسی (بخش ۱۷): GIN روی ستون نرمال‌شده‌ی اپ‌ساید (normalize-fa) با کانفیگ simple
CREATE INDEX "product_search_fts" ON "Product" USING GIN (to_tsvector('simple', "searchKey"));

-- ایندکس partial رزروهای فعال برای Worker انقضا (بخش ۴.۶)
CREATE INDEX "inventory_reservation_active_ttl" ON "InventoryReservation" ("expiresAt") WHERE "status" = 'ACTIVE';
